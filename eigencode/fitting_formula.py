...
def monte_carlo(tau0, xinit, c):
    ...
    plt.scatter(bincenters, n, marker='s', s=2, label=r'MC $\tau_0={}'.format(scinot(tau0).split(r'\times ')[1]), alpha=0.7, color=c)

def efunc(path, nmax, mmax, c):
    ...
    plt.plot(t/tlc, P*tlc, alpha=0.7, label=r'Eigenfunction expansion, $\tau_0={}'.format(scinot(tau0).split(r'\times ')[1]), c=c)

    # Fitting function
    expfalloff = -ssoln[0, 0] * Pnmsoln[0, 0] * np.exp(ssoln[0, 0] * t)
    tdiff = tlc * np.cbrt(p.a * p.tau0)
    plt.plot(t/tlc, np.exp(-(tdiff/t/tlc)**2) * expfalloff * tlc, '--', lw=2, alpha=0.7, label=r'Fitting function, $\tau_0={}'.format(scinot(tau0).split(r'\times ')[1]), c=c)
