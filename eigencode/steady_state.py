...
def fluence(sigma, p, Pnmsoln=None):
    ...
    '''
    xuniform = generate_xuniform(sigma, p)
    phi_xuniform = line_profile(xuniform**3 * p.c1, p)

    spec = np.zeros((p.nmax, np.shape(sigma))
    spec_xuniform = np.zeros((p.nmax, np.shape(xuniform)))
    phi = line_profile(sigma, p)

    if Pnmsoln is None:
        for n in range(1, p.nmax+1):
            spec[n-1] = -np.sqrt(6) * np.pi / 3. / p.k / p.Delta / phi 
        ...
        for n in range(1, p.nmax+1):
            for m in range(nsolnmax):
                spec[n-1] += 16. * np.pi**2 * p.radius * p.Delta 
