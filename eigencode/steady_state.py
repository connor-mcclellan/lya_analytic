...
def mfluence(sigma, p, Jsoln=None, ssoln=None, dijkstra=False):
    '''
    Calculates the fluence or luminosity by frequency eigenmode.
    '''
    #xuniform, sigma_to_x = generate_xuniform(sigma, p)
    #sigma_xuniform = xuniform**3 * p.c1
    #phi_xuniform = line_profile(sigma_xuniform, p)
...
if __name__ == '__main__':
    directory = Path('./data/210520-1705_wide_int_bounds_2').resolve()
    Jsoln, ssoln, intJsoln, p = construct_sol(directory, nmax=19, mmax=100)

    directory = Path('./data/210507_all').resolve()
    Jsoln2, ssoln2, intJsoln2, p2 = construct_sol(directory, nmax=19, mmax=100)
    ...
    for m in range(1, p.mmax):
        fig, ax = plt.subplots(1, 1)
        ax.plot(x_s, np.abs(np.sum(steady_state[:19], axis=0)), '-', marker='s', ms=1, alpha=0.7, label='steady state')
        ax.plot(x_d, np.abs(dijkstra[0]), '-', marker='s', ms=1, alpha=0.7, label=r'dijkstra')
        ax.plot(x_t, np.sum(tdep_spec[:m], axis=0), 'r-', marker='o', ms=1, alpha=0.7, label=r'2x wider integration bounds, $n < 19$, $m < 100$')
        ax.plot(x_t2, np.sum(tdep_spec2[:m], axis=0), 'm--', marker='^', ms=1, alpha=0.7, label=r'Regular, $n < 19$, $m < 100$')
        plt.yscale('log')
        plt.ylim(1e-16, 1e-10)
        plt.xlim(0, 30)
        plt.title('abs val of sum to n={}, m={}'.format(19, m))
        plt.xlabel('x')
        plt.legend()
        plt.tight_layout()
        plt.show()
        #plt.savefig('timedep_v_steadystate_m{:03d}.pdf'.format(m))
        plt.close()
