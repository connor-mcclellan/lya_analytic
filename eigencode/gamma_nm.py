...
directory = Path('./data/sweeptest_control').resolve()
nmin = 1
#Jsoln, ssoln, intJsoln, p = construct_sol(directory, nmax=4, mmax=100)
...
for n in range(nmin, nmax+1):
#    gamma_analytic = n**(-4/3)*gamma(n, np.arange(mmin, mmax+1), p)
#    gamma_sweep = -n**(-4/3)*ssoln[n-1][:mmax]
    gamma_analytic = gamma(n, np.arange(mmin, mmax+1), p)
    gamma_sweep = -ssoln[n-1][:mmax]
